			...
			if (body->wantsSleeping())
			{
				...
				else
				{
					...
					if (body->getActivationState() == ISLAND_SLEEPING)
					{
						...
						if(body->getEccentric())
						{
						    body->setEccentricLinearVelocity(btVector3(0, 0, 0));
						}
					}
				}
			}
			...
			if (getDispatchInfo().m_useContinuous && body->getCcdSquareMotionThreshold() && body->getCcdSquareMotionThreshold() < squareMotion)
			{
				...
				if (body->getCollisionShape()->isConvex())
				{
					...
					if (sweepResults.hasHit() && (sweepResults.m_closestHitFraction < 1.f))
					{
						...
						btVector3 eccLinVel = body->getEccentricLinearVelocity();
						...
						if (linVel.length2()>maxSpeedSqr)
						{
							...
							if(body->eccentric())
							    body->setLinearVelocity(eccLinVel);
		...
		for (int i = 0; i < m_predictiveManifolds.size(); i++)
		{
			...
			for (int p = 0; p < manifold->getNumContacts(); p++)
			{
				...
				//if (pt.getDistance()>0 && combinedRestitution>0 && pt.m_appliedImpulse != 0.f)
				{
					...
					if(!body0->getEccentric())
					    CoR1 = body0->getWorldTransform().getOrigin();
				    else
				        CoR1 = body0->getWorldTransform().getOrigin() + body0->getWorldEccentricity();
			        
			        if(!body1->getEccentric())
					    CoR2 = body1->getWorldTransform().getOrigin();
				    else
				        CoR2 = body1->getWorldTransform().getOrigin() + body1->getWorldEccentricity();
					
					btVector3 rel_pos0 = pos1 - CoR1;
					btVector3 rel_pos1 = pos2 - CoR2;
