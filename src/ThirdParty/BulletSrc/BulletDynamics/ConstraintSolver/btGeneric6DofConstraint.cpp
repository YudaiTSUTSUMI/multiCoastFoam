	...
	const btVector3& anchorPos)
{
	...
	btVector3 rel_pos1 = anchorPos - body1.getCenterOfMassPosition() - body1.getWorldEccentricity();
	btVector3 rel_pos2 = anchorPos - body2.getCenterOfMassPosition() - body2.getWorldEccentricity();
...
void btGeneric6DofConstraint::getInfo2(btConstraintInfo2* info)
{
	...
	const btVector3& linVelA = m_rbA.getEccentric() ? m_rbA.getEccentricLinearVelocity() : m_rbA.getLinearVelocity();
	const btVector3& linVelB = m_rbB.getEccentric() ? m_rbB.getEccentricLinearVelocity() : m_rbB.getLinearVelocity();
	...
	{  // if the joint is powered, or has joint limits, add in the extra row
		...
		if ((!rotational))
		{
			if (m_useOffsetForConstraintFrame)
			{
				...
				relB = m_calculatedTransformB.getOrigin() - transB.getOrigin() - m_rbB.getWorldEccentricity();
				// get its projection to constraint axis
				btVector3 projB = ax1 * relB.dot(ax1);
				// get vector directed from bodyB to constraint axis (and orthogonal to it)
				btVector3 orthoB = relB - projB;
				// same for bodyA
				relA = m_calculatedTransformA.getOrigin() - transA.getOrigin() - m_rbA.getWorldEccentricity();
			...
			else
			{
				...
				btVector3 c = m_calculatedTransformB.getOrigin() - transA.getOrigin() - m_rbA.getWorldEccentricity();
				ltd = c.cross(ax1);
				info->m_J1angularAxis[srow + 0] = ltd[0];
				info->m_J1angularAxis[srow + 1] = ltd[1];
				info->m_J1angularAxis[srow + 2] = ltd[2];

				c = m_calculatedTransformB.getOrigin() - transB.getOrigin() - m_rbB.getWorldEccentricity();
				ltd = -c.cross(ax1);
