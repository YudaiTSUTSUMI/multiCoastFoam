...
void btSequentialImpulseConstraintSolver::initSolverBody(btSolverBody* solverBody, btCollisionObject* collisionObject, btScalar timeStep)
{
	...
	if (rb)
	{
		...
		if(rb->getEccentric())
		{
		    solverBody->m_eccentric = rb->getEccentric();
		    solverBody->m_localEccentricity = rb->getLocalEccentricity();
	        solverBody->m_worldEccentricity = rb->getWorldEccentricity();
	        solverBody->m_eccentricWorldTransformOrigin = rb->getWorldTransform().getOrigin() + rb->getWorldEccentricity();
	        solverBody->m_linearVelocity = rb->getEccentricLinearVelocity();
		}
	}
	else
		{
	...
	for (int j = 0; j < manifold->getNumContacts(); j++)
	{
		...
		if (cp.getDistance() <= manifold->getContactProcessingThreshold())
		{
			...
			rel_pos2 = pos2 - colObj1->getWorldTransform().getOrigin();
			        ...
			        //new
			        if(solverBodyA->getEccentric())
			        {                           
			            rel_pos1 = pos1 - solverBodyA->getEccentricWorldTransformOrigin();
			            
			            if(cp.getDistance() < -0.00001)
			            {
			                printf("pos1 = %f %f %f\n",pos1.getX(),pos1.getY(),pos1.getZ());
			                btVector3 CoM = solverBodyA->getEccentricWorldTransformOrigin();
			                printf("CoM = %f %f %f\n",CoM.getX(),CoM.getY(),CoM.getZ());
			                printf("distance = %f\n",cp.getDistance());	
			            }		             
			        }
			        
			        if(solverBodyB->getEccentric())
			        {
			            rel_pos2 = pos2 - solverBodyB->getEccentricWorldTransformOrigin();
			            
			            if(cp.getDistance() < -0.00001)
			            {
			                printf("pos2 = %f %f %f\n",pos2.getX(),pos2.getY(),pos2.getZ());
			                btVector3 CoM = solverBodyB->getEccentricWorldTransformOrigin();
			                printf("CoM = %f %f %f\n",CoM.getX(),CoM.getY(),CoM.getZ());
			                printf("distance = %f\n",cp.getDistance());		
			            }
			        }
	...
	const btContactSolverInfo& infoGlobal)
{
	...
	for (int j = 0; j < info1.m_numConstraintRows; j++)
	{
...

		{
            ...
            btScalar vel1Dotn = solverConstraint.m_contactNormal1.dot(bodyAPtr->m_linearVelocity + externalForceImpulseA) + solverConstraint.m_relpos1CrossNormal.dot(bodyAPtr->m_angularVelocity + externalTorqueImpulseA);

			btScalar vel2Dotn = solverConstraint.m_contactNormal2.dot(bodyBPtr->m_linearVelocity + externalForceImpulseB) + solverConstraint.m_relpos2CrossNormal.dot(bodyBPtr->m_angularVelocity + externalTorqueImpulseB);
...
void btSequentialImpulseConstraintSolver::writeBackBodies(int iBegin, int iEnd, const btContactSolverInfo& infoGlobal)
{
	for (int i = iBegin; i < iEnd; i++)
	{
		...
		if (body)
		{
			...
			//changed
			btVector3 linearVelocitySetting;
			btVector3 angularVelocitySetting;
			btVector3 eccentricLinearVelocitySetting;
			
			if(!m_tmpSolverBodyPool[i].getEccentric()) 
			{
			    linearVelocitySetting = m_tmpSolverBodyPool[i].m_linearVelocity + m_tmpSolverBodyPool[i].m_externalForceImpulse;
			    angularVelocitySetting = m_tmpSolverBodyPool[i].m_angularVelocity + m_tmpSolverBodyPool[i].m_externalTorqueImpulse;
			}
			        else
			        {
			            eccentricLinearVelocitySetting = m_tmpSolverBodyPool[i].m_linearVelocity + m_tmpSolverBodyPool[i].m_externalForceImpulse;
			            angularVelocitySetting = m_tmpSolverBodyPool[i].m_angularVelocity + m_tmpSolverBodyPool[i].m_externalTorqueImpulse;
			            
			            // predict velocity
			                    btTransform dummyTransform = body->getWorldTransform();
			                    /*btQuaternion predictedOrn = body->getWorldTransform().getRotation();
                                predictedOrn += (angularVelocitySetting * predictedOrn) * (infoGlobal.m_timeStep * btScalar(0.5));
                                predictedOrn.normalize();
                                dummyTransform.setRotation(predictedOrn);*/
                                
                                btTransformUtil::integrateTransform(body->getWorldTransform(), eccentricLinearVelocitySetting, angularVelocitySetting, infoGlobal.m_timeStep, dummyTransform);
                                
                                btVector3 error = dummyTransform.getBasis() * body->getLocalEccentricity()
                                                     - body->getWorldEccentricity();
                                                     
			            linearVelocitySetting = eccentricLinearVelocitySetting - (1.0/infoGlobal.m_timeStep)*error;
			            //linearVelocitySetting = eccentricLinearVelocitySetting - (angularVelocitySetting).cross(m_tmpSolverBodyPool[i].getWorldEccentricity());
			        }
			
			m_tmpSolverBodyPool[i].m_originalBody->setLinearVelocity(linearVelocitySetting);
			m_tmpSolverBodyPool[i].m_originalBody->setAngularVelocity(angularVelocitySetting);
			if(m_tmpSolverBodyPool[i].getEccentric())
			{
			    m_tmpSolverBodyPool[i].m_originalBody->setEccentricLinearVelocity(eccentricLinearVelocitySetting);
		    }
